...
θL_list = []
for s ∈ 1:sample
    θlength = vcat(ones(Int, length(fold)) * 2, ones(Int, length(fold)) * 2) 
...
filename *= "_samples"*string(sample)*"_YXXY.txt"
open(filename, "w") do io
    ...
    for f ∈ 1:length(fold)
        if fold[f] == 1
            println(io, "layer "*string(f)*": ", θL_list[idx][f], " (unitary, YXXY type),  ", θL_list[idx][f+length(fold)], " (isometry, YXXY type)")
        elseif fold[f] >= 1
            println(io, "layer "*string(f)*"-"*string(f+fold[f]-1)*": ", θL_list[idx][f], " (unitary, YXXY type),  ", θL_list[idx][f+length(fold)], " (isometry, YXXY type)")
        end
        l += fold[f]
    end
    println(io, "\n\nsum|angles|=",sa)
